...
class Blade2DConnectingArcs:
    ...
    def __init__(self, section_variables):
        ...
        self.x3 = (
            ...
            - self.radius_out * (1 - np.sin(self.theta3))
        )
        self.y3 = (
            self.y_in
            + self.chord * np.sin(self.stagger)
            - self.radius_out * np.cos(self.theta3)
        )
        ...
        self.x4 = (
            self.x_in
            + self.chord * np.cos(self.stagger)
            - self.radius_out * (1 + np.sin(self.theta4))
        )
        self.y4 = (
            self.y_in
            + self.chord * np.sin(self.stagger)
            + self.radius_out * np.cos(self.theta4)
        )
    ...
    def get_section_coordinates(self, u):
        ...
        section_coordinates = np.concatenate(
            (coordinates_12, coordinates_23, coordinates_34, coordinates_41), axis=1
        )
    ...
    def make_leading_edge(self):
        ...
        P1 = (P0 + P2) / 2 - R * np.cos(a) / np.tan(
            a
        ) * n  # Second control point (see notes)
    ...
    def make_upper_side(self):
        ...
        P1 = np.array([self.x4, self.y4]) - self.dist_4 * self.chord * np.array(
            [np.cos(self.theta4), np.sin(self.theta4)]
        )
        P2 = np.array([self.x1, self.y1]) + self.dist_1 * self.chord * np.array(
    ...
    def plot_blade(
        ...
        if blade_section == "yes":
            ...
            # Plot leading edge
            (line,) = ax.plot(
                np.real(self.leading_edge.get_NurbsCurve_value(u_plot)[0, :]),
            ...
            # Plot lower_surface
            (line,) = ax.plot(
                np.real(self.lower_side.get_NurbsCurve_value(u_plot)[0, :]),
            ...
            # Plot trailing edge
            (line,) = ax.plot(
                np.real(self.trailing_edge.get_NurbsCurve_value(u_plot)[0, :]),
            ...
            (line,) = ax.plot(
                np.real(self.upper_side.get_NurbsCurve_value(u_plot)[0, :]),
        ...
        if control_points == "yes":
            ...
            (line,) = ax.plot(
                np.real(self.leading_edge.P[0, :]), np.real(self.leading_edge.P[1, :])
            )
            ...
            (line,) = ax.plot(
                np.real(self.lower_side.P[0, :]), np.real(self.lower_side.P[1, :])
            )
            line.set_linewidth(0.75)
            line.set_linestyle("-.")
            line.set_color("r")
            ...
            (line,) = ax.plot(
                np.real(self.trailing_edge.P[0, :]), np.real(self.trailing_edge.P[1, :])
            )
            line.set_linewidth(0.75)
            line.set_linestyle("-.")
            line.set_color("r")
    ...
    def plot_blade_cascade(self, fig=None, ax=None):
        ...
        for k in range(3):
            ...
            # Draw blade surface
            (line,) = ax.plot(
                np.real(self.get_section_coordinates(u_plot)[0, :]),
                np.real(self.get_section_coordinates(u_plot)[1, :])
                + self.spacing * (k - 1),
            )
            line.set_linewidth(1.25)
            line.set_linestyle("-")
