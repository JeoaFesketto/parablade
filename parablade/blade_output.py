...
class BladeOutput:
        ...
        if self.OPERATION_TYPE == "SENSITIVITY":
            ...
            for key in self.surface_sensitivity.keys():
                ...
                if self.NDIM == 3:
                    ...
                    for i in range(self.N_points):
                        file.write(
                            "%i,\t%+.16e,\t%+.16e,\t%+.16e,\t%+.16e,\t%+.16e,\t%+.16e,\t%+.16e,\t%+.16e,\t%+.16e\n"
                            % (
                                i,  # Index
                    ...
                    for i in range(self.N_points):
                        file.write(
                            "%i,\t%+.16e,\t%+.16e,\t%+.16e,\t%+.16e,\t%+.16e,\t%+.16e\n"
                            % (
                                i,  # Index
                                self.surface_coordinates[0, i],  # x coordinate (axial)
                                self.surface_coordinates[
                                    1, i
                                ],  # y coordinate (tangential)
    ...
    def make_TurboGrid_meshfile(self):
        ...
        for v_section in v:
            ...
            section_coordinates = np.real(
                self.blade_in.get_surface_coordinates(
                    u=u, v=np.ones(n_points) * v_section
                )
            )
